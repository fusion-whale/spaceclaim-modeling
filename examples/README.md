...
| 示例 | 是什么 | 用到的能力 | 实测结果 |
|---|---|---|---|
| `pche_demo.py` | **PCHE 一对冷热通道**：D 形（半圆槽），流向沿 x、**平边朝 +z**，长 100mm，冷热**上下对齐**；固体域 = 刻槽板 + 盖板 | `half_round_channel`（空白处造好再 move）/ 逐层浇注切槽 / `cht_check` | **3 个体**：固体 100×3.2×3.8（**10 面**）、冷热流体各 100×2×1（4 面）；**Dh = 1.2220 mm**；交界面各 2 对、两侧各 **514.16 mm²**、balanced=True；冷热薄壁 0.6mm；`cht_check ok=True` |
