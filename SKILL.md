...
② 切热通道的 D 形槽                  -> 刀心正好落在板上表面 z=1.6，只切掉下半 = 真半圆
③ 浇 1.6mm 冷板（与热板并成一个固体）
④ 切冷通道的 D 形槽                  -> 刀心在 z=3.2，热槽 z 0.6~1.6 不受影响
⑤ 浇 0.6mm 盖板把冷通道盖住          -> 固体域 1 个体、2 条封闭 D 形通道
⑥ half_round_channel() 造 2 条流体域 -> 空白处造好再 move 进来
```

步骤 ②④ 能成立的关键：**刀心正好落在被切体的上表面**，刀的上一半在体外，
所以切出来的是真半圆（解析曲面），而不是"整圆孔"。

尺寸（按真实 PCHE 量级，**一对**通道）：通道半径 1.0mm、**D 的平边朝 +z**（弧往 -z 鼓）、
刻槽板 1.6mm、盖板 0.6mm、板宽 3.2mm（通道 2 + 两侧各 0.6）、**冷热上下对齐**
→ 总厚 3.8mm，**冷热之间只剩 0.6mm 的换热薄壁**。

实测：

| 项 | 数值 |
|---|---|
| 体数 | **3** = 1 固体（**10 面** = 6 外壳 + 2 通道×2）+ 1 热流体 + 1 冷流体（各 4 面） |
| D 形截面 | 面积 **1.5708 mm²**、湿周 **5.1416 mm**、**Dh = 1.2220 mm** |
| 交界面 | hot / cold 各 **2 对**（每条通道：半圆弧面 314.16 + 平顶面 200）→ 两侧各 **514.16 mm²**、`balanced=True` |
| 固体体积 | **901.84 mm³** = 100×3.2×3.8 − 2×157.08 |
