...
## 27. D 形通道、PCHE 与分段匹配的一个假阳性（第 22 轮）

### 27.1 全局 cut 的死角，与"空白处造好再搬"的解法

想造一个**半圆柱**（D 形通道）时会撞上一条硬限制：**`cut=True` 是全局的，没法只切某一个体**。

- 在目标位置把整圆柱砍一半 → 那一刀会把周围的固体一起砍掉 ✗
- 先把固体的槽切好、再造流体域 → 造流体域时那一刀又会伤到固体 ✗
- 先造流体域、再造固体 → 固体会跟它并集（或重叠）✗

解法（`half_round_channel()` 的做法）：

```
1) 把整圆柱造在**文档包围盒之外**的空白处（separate=True）
2) 在那里用 box(cut=True) 切掉一半 —— 此时文档里只有它，随便切
3) move() 搬回目标位置 —— 刚体变换，**不会并集**（实测过，见 §22.2）
```

搬回去之后，半圆柱与固体的槽面完全重合 → 正好是一对交界面。

```
half_round_channel(radius, length, origin, axis="x", flat="+y", name="Channel")
```

`origin` 是**平的那一面**的中心；通道朝 flat 的反方向鼓出 radius。
`flat` 取 `"+y" / "-y" / "+z" / "-z"`，且不能与 `axis` 同向。

### 27.2 固体槽怎么切出"真半圆"

关键技巧：**刀心正好落在被切体的上表面**。刀（整圆柱）的上一半在体外，所以切出来
只剩下半 —— 得到的是**解析半圆面**，不是"整圆孔"。

```
① box(100, 1.6, 6.8)               # 热板，上表面 y=1.6
② cylinder(1.0, 110, (-5, 1.6, z), axis="x", cut=True)   # 刀心在 y=1.6 -> 只切下半
③ box(100, 1.6, 6.8, origin=(0,1.6,0))                   # 冷板，与热板并成一个固体
④ cylinder(1.0, 110, (-5, 3.2, z), axis="x", cut=True)   # 冷槽在板上部，热槽不受影响
⑤ box(100, 0.6, 6.8, origin=(0,3.2,0))                   # 盖板，把冷槽盖住
⑥ half_round_channel() x 4                                # 流体域
```

### 27.3 实测（`examples/pche_demo.py`）

| 项 | 数值 |
|---|---|
| 体数 | 5 = 1 固体（**14 面** = 6 外壳 + 4 通道 x 2） + 2 热流体 + 2 冷流体（各 4 面） |
| D 形截面 | 面积 1.5708 mm²、湿周 5.1416 mm、Dh = 1.2220 mm |
| 交界面 | hot / cold 各 4 对、两侧各 1028.32 mm²、balanced=True |
| 固体体积 | 1955.68 mm³ = 2584 − 4×157.08 |
| `cht_check` | ok=True；热↔冷正确判为"不接触" |

单条通道的交界面 = 半圆弧面 πR·L = 314.16 + 平顶面 2R·L = 200 = **514.16 mm²**；
两条 = 1028.32 ✓ 与手算逐位相等。

### 27.4 分段匹配的假阳性：带内环的面不能做包含判定

`interface_report(..., allow_split=True)` 一开始把流体域的**半圆端面**配成了交界面：
它和固体端面（x=0 那张面）共面、包围盒也被罩住 —— 但它其实落在固体端面的**孔里**
（通道从端面穿出，在那张面上留下 4 个半圆孔），并不贴着材料。
实测多出 **4 对假交界面、面积多算 6.28 mm²**（= 4 × 1.5708，正是半圆端面的面积）。

修法：`_face_contains()` 里先判 **大面必须只有 1 个环**（`Loops.Count == 1`）。带孔的面
直接不做包含判定 —— 在那种面上"包围盒罩住"和"贴着材料"根本不是一回事。
`loops` 本来就在 `match_faces` 里用过，判定很便宜。

修完 hot 交界面从「4 整 + 4 分段 / 1034.60」回到「**4 对 / 1028.32**」，
与手算完全一致。