...
④ cylinder(1.0, 110, (-5, 1.6, 3.2), axis="x", cut=True) # 刀心在 z=3.2，热槽不受影响
⑤ box(100, 3.2, 0.6, origin=(0,0,3.2))                   # 盖板，把冷槽盖住
⑥ half_round_channel(flat="+z") x 2                      # 流体域（冷热各一条，对齐）
```

注意这里 **D 的平边朝 +z**（法向 +z、弧往 -z 鼓），所以切槽的刀心落在**板上表面**
那一侧；板是沿 z 叠的。
...
| 项 | 数值 |
|---|---|
| 体数 | **3** = 1 固体（**10 面** = 6 外壳 + 2 通道 x 2） + 1 热流体 + 1 冷流体（各 4 面） |
| D 形截面 | 面积 1.5708 mm²、湿周 5.1416 mm、Dh = 1.2220 mm |
| 交界面 | hot / cold 各 **2 对**、两侧各 **514.16 mm²**、balanced=True |
| 固体体积 | **901.84 mm³** = 1216 − 2×157.08 |
